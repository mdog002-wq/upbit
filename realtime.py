...
<body>
    ...
    <script>
    // A 사이트의 추천 종목 데이터를 안전하게 가져오는 함수 (CORS 및 JSON 데이터 응답 형태 대응)
    async function fetchARecommendations() {
      try {
        const response = await fetch('https://upbit-a.onrender.com/api/rankings', {
          method: 'GET',
          headers: { 'Accept': 'application/json' }
        });
        if (!response.ok) throw new Error("A 사이트 API 응답 오류");
        
        const json = await response.json();
        
        // 다양한 응답 JSON 구조 대응 (배열, {data: []}, {rankings: []} 등)
        if (Array.isArray(json)) return json;
        if (json.data && Array.isArray(json.data)) return json.data;
        if (json.rankings && Array.isArray(json.rankings)) return json.rankings;
        if (json.result && Array.isArray(json.result)) return json.result;

        return [];
      } catch (e) {
        console.warn("A 사이트 추천 데이터를 불러오지 못했습니다.", e);
        return [];
      }
    }
    ...
    async function renderRecommendedListSortedByR() {
      ...
      container.innerHTML = '<div style="text-align: center; color: #6c757d; padding: 15px 0;">데이터 로딩 중...</div>';

      const aItems = await fetchARecommendations();

      if (!aItems || aItems.length === 0) {
        container.innerHTML = '<div style="text-align: center; color: #dc3545; padding: 10px; font-size: 11px;">추천 종목 데이터를 불러올 수 없습니다.<br>(A 사이트 CORS 및 API 확인 필요)</div>';
        return;
      }

      // A 사이트 추천 종목 코인들에 R 사이트 현재 순위 매핑
      const mappedList = aItems.map(aCoin => {
        // rawTicker 추출 (KRW- 제거 및 대문자 정규화)
        const rawTicker = (aCoin.ticker || aCoin.symbol || aCoin.market || '').replace('KRW-', '').toUpperCase();
        const marketKey = `KRW-${rawTicker}`;

        // R 사이트(analysisData)에서 동일 코인 검색
        const rMatch = analysisData.find(r => 
          r.ticker.toUpperCase() === rawTicker || 
          r.market.toUpperCase() === marketKey
        );

        return {
          name: aCoin.name || (rMatch ? rMatch.name : rawTicker),
          ticker: rawTicker,
          market: marketKey,
