...
DOCS_HISTORY_FILE = os.path.join(DOCS_DIR, "history_db.json")
...
def save_json(filepath, data):
    try:
        ...
        print(f"⚠️ JSON 저장 실패 ({filepath}): {e}")
        return False
...
def analyze_single_coin(market, k_name, golden_price_patterns, golden_vol_patterns, weights, recommended_symbols):
    try:
        ...
        # 동적 가중치 산출
        high_max = df["high_price"].max() or current_price
        breakout_val = (current_price / high_max) * 100.0
        ...
        base_score = (
            ...
            breakout_val * weights.get("w_breakout", 0.05)
        )
...
def main():
    ...
    try:
        res = requests.get("https://api.upbit.com/v1/market/all", timeout=5)
    ...
    top20 = analyzed_results[:20]

    # data/ 와 docs/ 두 폴더 모두에 결과 저장 (GitHub Pages 연동)
    save_json(HISTORY_FILE, top20)
    save_json(DOCS_HISTORY_FILE, top20)

    if top20:
        print(f"✅ 2차 진화형 실시간 스코어링 완료 (1위: {top20[0]['ticker']} - {top20[0]['score']}점)")
    else:
        print("⚠️ 분석 결과가 없습니다.")
