...
def fetch_ai_recommendations():
    """레포1 (mdog002-wq/upbit-a)의 AI 추천 분석 결과를 원격 가져오기"""
    refined_set = set()
...
def analyze_single_coin(market, k_name, ideal_price_pattern, ideal_vol_pattern, history_db, weights, btc_multiplier, ws_data, is_ai_recommended, is_warning):
    ...
    ai_volatility_score = float(min(100.0, (df_2h["candle_acc_trade_volume"].std() / (df_2h["candle_acc_trade_volume"].mean() + 1e-8)) * 50))
    ...
    base_score = (
        ...
        vol_cliff_score * weights.get("w_vol_cliff", 0.03) +
        breakout_score * weights.get("w_breakout", 0.20) +
        vol_surge_score * weights.get("w_vol_surge", 0.25) +
        ma_momentum_score * weights.get("w_ma_alignment", 0.12) +
        min(100.0, max(0.0, change_rate * 3.33)) * weights.get("w_daily_momentum", 0.20)
    )

    if rsi >= 72.0 or rsi <= 35.0: base_score *= 0.80
    if liquidity_index < 10.0: base_score *= 0.70
    ...
    if corpse_ratio >= 40.0: final_score *= 0.97
    ...
    calculated_sl = min(current_price * 0.985, min(current_price - (atr * 1.2), df_2h["low_price"].min() * 0.995))
...
def main():
    ...
    weights = load_json(WEIGHTS_FILE, {
        "w_pattern": 0.10, "w_buy_sell": 0.05, "w_ai_volatility": 0.05,
        "w_vol_cliff": 0.10, "w_breakout": 0.15, "w_vol_surge": 0.20,
        "w_ma_alignment": 0.10, "w_daily_momentum": 0.25
    })
